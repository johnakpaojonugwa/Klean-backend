...
export const payrollService = {
    ...
    // Process payroll for all employees in a branch with transaction support
    processPayrollForBranch: async (branchId, payrollMonth, processedBy) => {
        ...
        try {
            ...
            for (const employee of employees) {
                ...
                // Create payroll record within transaction
                ...
                payrolls.push(payroll[0]);
            }

            // Commit transaction only if all payrolls were created successfully
            await session.commitTransaction();
            logger.info(`Payroll processed for ${payrolls.length} employees in branch ${branchId} (TRANSACTION COMMITTED)`);
            return payrolls;
        } catch (error) {
            // Rollback transaction on error
            await session.abortTransaction();
            logger.error("Payroll processing error (TRANSACTION ROLLED BACK):", error.message);
            throw new Error(`Payroll processing failed. No changes were made. Details: ${error.message}`);
        } finally {
